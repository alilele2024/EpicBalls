...
struct ThirdLevelView: View {
    ...
    @StateObject private var gameScene = ThirdGameSceneWrapper()
    ...
    private func handleGameEvent(_ event: ThirdGameEvent?) {
        switch event {
        case .hitTopBall:
    ...
    private func endGame(with message: String) {
        ...
        if message == "complete" {
            isCompleted = true
            UserDefaults.standard.setValue(true, forKey: "thirdLevelCompleted")
        }
        isGameOver = true
...
class ThirdGameSceneWrapper: ObservableObject {
    @Published var scene: ThirdGameScene
    @Published var gameEvent: ThirdGameEvent? = nil
    
    init() {
        scene = ThirdGameScene()
...
class ThirdGameScene: SKScene, SKPhysicsContactDelegate {
    var gameEventHandler: ((ThirdGameEvent) -> Void)?
