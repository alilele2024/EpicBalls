...
struct SecondLevelView: View {
    ...
    @StateObject private var gameScene = SecondGameSceneWrapper()
    ...
    private func handleGameEvent(_ event: SecondGameEvent?) {
        switch event {
        case .hitTopBall:
    ...
    private func endGame(with message: String) {
        ...
        if message == "complete" {
            isCompleted = true
            UserDefaults.standard.setValue(true, forKey: "secondLevelCompleted")
        }
        isGameOver = true
...
class SecondGameSceneWrapper: ObservableObject {
    @Published var scene: SecondGameScene
    @Published var gameEvent: SecondGameEvent? = nil
    
    init() {
        scene = SecondGameScene()
...
class SecondGameScene: SKScene, SKPhysicsContactDelegate {
    var gameEventHandler: ((SecondGameEvent) -> Void)?
    ...
    private func createStripe() {
       ...
       // stripe = SKSpriteNode(color: .red, size: CGSize(width: 100, height: 10))
        stripe.position = CGPoint(x: frame.midX, y: frame.height / 2)
        stripe.name = "stripe"
    ...
    private func moveStripe() {
        let moveRight = SKAction.moveTo(x: frame.width - stripe.size.width / 2, duration: 2)
        let moveLeft = SKAction.moveTo(x: stripe.size.width / 2, duration: 2)
    ...
    private func moveTopBalls() {
        ...
        for (index, ball) in topBalls.enumerated() {
            ...
            let newXPosition = leftMargin + (rightMargin - leftMargin) / CGFloat(topBalls.count - 1) * CGFloat(index)
            let moveRight = SKAction.moveTo(x: rightMargin, duration: TimeInterval((rightMargin - newXPosition) / currentTopBallSpeed))
            let moveLeft = SKAction.moveTo(x: leftMargin, duration: TimeInterval((newXPosition - leftMargin) / currentTopBallSpeed))
            let sequence = SKAction.sequence([moveRight, moveLeft])
