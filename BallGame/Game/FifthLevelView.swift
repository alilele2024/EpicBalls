...
struct FifthLevelView: View {
    @Environment(\.dismiss) var dismiss
    @StateObject private var gameScene = FifthGameSceneWrapper()
    ...
    private func handleGameEvent(_ event: FifthGameEvent?) {
        switch event {
        case .hitTopBall:
    ...
    private func endGame(with message: String) {
        ...
        if message == "complete" {
            isCompleted = true
            UserDefaults.standard.setValue(true, forKey: "fifthLevelCompleted")
        }
        isGameOver = true
...
class FifthGameSceneWrapper: ObservableObject {
    @Published var scene: FifthGameScene
    @Published var gameEvent: FifthGameEvent? = nil
    
    init() {
        scene = FifthGameScene()
...
class FifthGameScene: SKScene, SKPhysicsContactDelegate {
    var gameEventHandler: ((FifthGameEvent) -> Void)?
