...
struct FirstLevelView: View {
    ...
    @StateObject private var gameScene = FirstGameSceneWrapper()
    ...
    private func handleGameEvent(_ event: FirstGameEvent?) {
        switch event {
        case .hitTopBall:
    ...
    private func endGame(with message: String) {
        ...
        if message == "complete" {
            isCompleted = true
            UserDefaults.standard.setValue(true, forKey: "firstLevelCompleted")
        }
        isGameOver = true
...
class FirstGameSceneWrapper: ObservableObject {
    @Published var scene: FirstGameScene
    @Published var gameEvent: FirstGameEvent? = nil
...
class FirstGameScene: SKScene, SKPhysicsContactDelegate {
    var gameEventHandler: ((FirstGameEvent) -> Void)?
    ...
    private func createBottomBall() {
        ...
        let textureName = userDefaults.string(forKey: "ball") ?? "el6" // "defaultTexture" — запасная текстура
